...
export class Joplin implements INodeType {
	...
	methods = {
		...
		listSearch: {
			...
			async searchNotes(this: ILoadOptionsFunctions,): Promise<INodeListSearchResult> {
			...
			async searchTags(this: ILoadOptionsFunctions,): Promise<INodeListSearchResult> {
				// Creds
			...
			try {
				if (resource === 'folder') {
					...
					if (operation === 'create') {
						...
						const flattenedFields = flattenObject(this.getNodeParameter('additionalFields', i));
						...
						folderProperties.title = this.getNodeParameter('name', i, undefined) as string;
						...
						const folder_id = this.getNodeParameter('folder_id', i, undefined) as string;
					...
					if (operation === 'create') {
						...
						// Inject required field message ID into payload
						noteProperties.title = this.getNodeParameter('title', i, undefined) as string;
						...
						//              note: delete
						// ----------------------------------------
						const noteId = this.getNodeParameter('noteId.value', i, '', undefined) as string;
						...
						noteProperties.id = this.getNodeParameter('noteId.value', i, '', undefined) as string;
					...
					if (operation === 'create') {
						...
						const flattenedFields = flattenObject(this.getNodeParameter('title', i));
						const filteredFields = filterValidProperties(flattenedFields);
						...
						tagAssign.tagId = this.getNodeParameter('tagId.value', i, '', undefined) as string;
						tagAssign.noteId = this.getNodeParameter('noteId.value', i, '', undefined) as string;
